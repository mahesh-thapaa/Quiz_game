...
class StadiumQuizScreen extends StatefulWidget {
  const StadiumQuizScreen({super.key});

  @override
  State<StadiumQuizScreen> createState() => _StadiumQuizScreenState();
}
...
class _StadiumQuizScreenState extends State<StadiumQuizScreen> {
  ...
  List<StadiumLevelTile> _generateLevelBlock({required int startPos}) {
    final List<StadiumLevelTile> items = [];
    for (int i = 0; i < 24; i++) {
      int gridPos = startPos + i;
      // Bonus positions: 6, 12, 18, 24, 30, 36...
      if (gridPos % 6 == 0) {
        items.add(
          StadiumLevelTile(hasStar: true, number: gridPos, starsEarned: 0),
        );
      } else {
        items.add(
          StadiumLevelTile(
  ...
  Future<void> _fetchAllLevelsAndQuestions() async {
    try {
      ...
      final savedProgress = await LevelProgressService.loadAllLevelStars(
        category: 'stadium_quiz',
      );

      // Fetch Stadium Quiz using dynamic category query
      final quizSnap = await FirebaseFirestore.instance
          .collection('quizzes')
          .where('category', isEqualTo: 'Stadium Quiz')
          .limit(1)
          .get();

      if (quizSnap.docs.isEmpty) {
        debugPrint('Stadium Quiz not found in Firestore');
        setState(() => _loadingQuestions = false);
        return;
      }

      String stadiumQuizDocId = quizSnap.docs.first.id;
      final levelsSnap = await FirebaseFirestore.instance
          .collection('quizzes')
          .doc(stadiumQuizDocId)
          .collection('levels')
          .orderBy('order')
          .get();
  ...
  void _unlockNextTile(StadiumLevelTile current) {
    final all = [...block1Items, ...block2Items];
            ...
            onTap: () {
              ...
              showBonusLevelSheet(
                ...
                onPlay: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => StadiumQuizGameplayScreen(
                      questions: questions,
                      levelNumber: gridPos,
          ...
          onTap: () {
            ...
            showLevelOverview(
              ...
              model: LevelOverviewModel(
                ...
                levelName: 'STADIUM QUIZ',
                description: 'Can you identify the stadium correctly?',
              ),
              onPlay: () => Navigator.push(
                context,
                MaterialPageRoute(
