...
class ClubQuizScreen extends StatefulWidget {
  const ClubQuizScreen({super.key});

  @override
  State<ClubQuizScreen> createState() => _ClubQuizScreenState();
}

class _ClubQuizScreenState extends State<ClubQuizScreen> {
  late List<ClubLevelTile> block1Items;
  late List<ClubLevelTile> block2Items;
  ...
  List<ClubLevelTile> _generateLevelBlock({required int startPos}) {
    final List<ClubLevelTile> items = [];
    for (int i = 0; i < 24; i++) {
      int gridPos = startPos + i;
      // Bonus positions: 6, 12, 18, 24, 30, 36...
      if (gridPos % 6 == 0) {
        items.add(
          ClubLevelTile(hasStar: true, number: gridPos, starsEarned: 0),
        );
      } else {
        items.add(
          ClubLevelTile(
  ...
  Future<void> _fetchAllLevelsAndQuestions() async {
    try {
      ...
      // Fetch Club Quiz using dynamic category query
      final quizSnap = await FirebaseFirestore.instance
          .collection('quizzes')
          .where('category', isEqualTo: 'Club Quiz')
          .limit(1)
          .get();

      if (quizSnap.docs.isEmpty) {
        debugPrint('Club Quiz not found in Firestore');
        setState(() => _loadingQuestions = false);
        return;
      }

      String clubQuizDocId = quizSnap.docs.first.id;
  ...
  void _unlockNextTile(ClubLevelTile current) {
    final all = [...block1Items, ...block2Items];
  ...
  Widget _buildGridSection(List<ClubLevelTile> items) {
    return SliverGrid(
            ...
            onTap: () {
              ...
              showBonusLevelSheet(
                ...
                onPlay: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => ClubQuizGameplayScreen(
                      questions: questions,
                      levelNumber: gridPos,
          ...
          onTap: () {
            ...
            showLevelOverview(
              ...
              model: LevelOverviewModel(
                ...
                levelName: 'CLUB CHALLENGE',
                description: 'Can you identify the club correctly?',
              ),
              onPlay: () => Navigator.push(
                context,
                MaterialPageRoute(
